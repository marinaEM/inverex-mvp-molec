# Foundation Model: Mini vs Full Comparison

## Overview

The INVEREX foundation model has two versions: the **mini pilot** (mini pilot) and the **full model** (full model). This document explains why the mini pilot is insufficient and what the full model adds.

## Mini Pilot (mini pilot)
...
The mini evaluated on only 10 representative datasets and compared against only 3 baselines. The full model evaluates on all labeled datasets and compares against 6+ baselines including the known LightGBM + mutations/clinical baseline (AUC 0.624).
